...
def main():
    ...
    init_xyz = ensure_xyz(input_file, workdir, job_name=job_name)
    ...
    try:
        if engine == "xtb":
            ...
            mult = int(job_data.get("mult", 1))
            mult_mode = str(job_data.get("mult_mode", "1_restricted"))
            if mult == 1:
                if mult_mode == "1_unrestricted":
                    uhf_val = 0
                else:
                    uhf_val = None
            else:
        ...
        if init_xyz and init_xyz.exists():
            try:
                init_xyz.unlink()
            except Exception as e:
                print(f"Warning: Failed to remove temporary file {init_xyz}: {e}")
